...
public class SubCategoryReport extends Report {
    ...
    public ReportData getReport(final DatabaseAdapter db, WhereFilter filter) {
        ...
        try {
            ...
            CategoryTree<CategoryAmount> amounts = CategoryTree
                    .createFromCursor(c, new NodeCreator<CategoryAmount>() {
                        @Override
                        public CategoryAmount createNode(Cursor c) {
                            BigDecimal amount;
                            try {
                                amount = TransactionsTotalCalculator.getAmountFromCursor(db, c, currency, rates,
                                        c.getColumnIndex(DatabaseHelper.ReportColumns.DATETIME));
    ...
    private void createStyles(Context context) {
        styles[0] = new GraphStyle.Builder(context).dy(2).textDy(5).lineHeight(30).nameTextSize(14).amountTextSize(12)
                .indent(0).build();
        styles[1] = new GraphStyle.Builder(context).dy(2).textDy(5).lineHeight(20).nameTextSize(12).amountTextSize(10)
                .indent(10).build();
        styles[2] = new GraphStyle.Builder(context).dy(2).textDy(5).lineHeight(20).nameTextSize(12).amountTextSize(10)
                .indent(30).build();
    }
