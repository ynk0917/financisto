...
public class CsvExport extends Export {
    ...
    private void writeLine(Csv.Writer w, Transaction t) {
        ...
        if (t.isTransfer()) {
            ...
            writeLine(w, dt, fromAccount.title, t.fromAmount, fromAccount.currency.id, 0, 0, category, null,
                    TRANSFER_OUT, project, t.note);
            writeLine(w, dt, toAccount.title, t.toAmount, toAccount.currency.id, 0, 0, category, null, TRANSFER_IN,
                    project, t.note);
        } else {
            MyLocation location = getLocationById(t.locationId);
            Payee payee = getPayee(t.payeeId);
            writeLine(w, dt, fromAccount.title, t.fromAmount, fromAccount.currency.id, t.originalFromAmount,
                    t.originalCurrencyId,
