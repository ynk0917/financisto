...
public class PlannerTest extends AbstractDbTest {
    ...
    private void assertTransactions2(List<TransactionInfo> transactions, Object... data) {
        ...
        for (int i = 0; i < count; i++) {
            assertTransaction2("Row " + (i + (Integer) data[i * 4]), transactions.get(i), (DateTime) data[i * 4 + 1],
                    (Integer) data[i * 4 + 2], (String) data[i * 4 + 3]);
        }
    }
    ...
    private void logTransactions(List<TransactionInfo> transactions) {
        ...
        for (TransactionInfo transaction : transactions) {
            Log.d("PlannerTest", df.format(new Date(transaction.dateTime)) + " " + Utils
                    .amountToString(Currency.EMPTY, transaction.fromAmount) + " " + transaction.note);
        }
        Log.d("PlannerTest", "==========");
    }
    ...
    private void prepareData() {
        ...
        // payment
        //t6
        TransactionBuilder.withDb(db).dateTime(date(2011, 8, 15).atNoon()).account(a1).amount(400).ccPayment()
                .note("t6").create();

        //scheduled once
        //t7
        TransactionBuilder.withDb(db).scheduleOnce(date(2011, 8, 14).atNoon()).account(a1).amount(-100).note("t7")
                .create();
        ...
        TransactionBuilder.withDb(db)
                .scheduleRecur("2011-08-02T23:00:00~WEEKLY:days@TUE#interval@1#~INDEFINETELY:null")
        ...
        //r6
        TransactionBuilder.withDb(db)
                .scheduleRecur("2011-08-02T22:30:00~WEEKLY:days@FRI#interval@1#~INDEFINETELY:null")
                .account(a2).amount(-120)
