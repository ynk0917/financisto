...
public class ScheduledAlarmReceiver extends PackageReplaceReceiver {
    ...
    private void requestAutoBackup(Context context) {
        Intent serviceIntent = new Intent(FinancistoService.ACTION_AUTO_BACKUP, null, context,
                FinancistoService.class);
        FinancistoService.enqueueWork(context, serviceIntent);
    }

    private void requestScheduleOne(Context context, Intent intent) {
        Intent serviceIntent = new Intent(FinancistoService.ACTION_SCHEDULE_ONE, null, context,
                FinancistoService.class);
        serviceIntent.putExtra(RecurrenceScheduler.SCHEDULED_TRANSACTION_ID,
