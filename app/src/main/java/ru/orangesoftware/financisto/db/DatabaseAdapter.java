...
public class DatabaseAdapter extends MyEntityManager {
    ...
    private static final String GET_PARENT_SQL = "(SELECT "
            ...
            + " WHERE "
            + " node." + CategoryColumns.left + " BETWEEN parent." + CategoryColumns.left + " AND parent."
            + CategoryColumns.right
            + " AND node." + CategoryColumns._id + "=?"
            + " AND parent." + CategoryColumns._id + "!=?"
            + " ORDER BY parent." + CategoryColumns.left + " DESC)";

    private static final String INSERT_CATEGORY_UPDATE_RIGHT = "UPDATE " + CATEGORY_TABLE + " SET "
            + CategoryColumns.right + "=" + CategoryColumns.right + "+2 WHERE " + CategoryColumns.right + ">?";

    private static final String INSERT_CATEGORY_UPDATE_LEFT = "UPDATE " + CATEGORY_TABLE + " SET "
            + CategoryColumns.left + "=" + CategoryColumns.left + "+2 WHERE " + CategoryColumns.left + ">?";

    private static final String CATEGORY_UPDATE_CHILDREN_TYPES = "UPDATE " + CATEGORY_TABLE + " SET "
            + CategoryColumns.type + "=? WHERE " + CategoryColumns.left + ">? AND " + CategoryColumns.right + "<?";
    ...
    private static final String UPDATE_LAST_RECURRENCE =
            "UPDATE " + TRANSACTION_TABLE + " SET " + TransactionColumns.last_recurrence + "=? WHERE "
    ...
    public int deleteAccount(long id) {
        ...
        try {
            ...
            db.delete(TRANSACTION_ATTRIBUTE_TABLE, TransactionAttributeColumns.TRANSACTION_ID
                    + " in (SELECT _id from " + TRANSACTION_TABLE + " where " + TransactionColumns.from_account_id
                    + "=?)", sid);
    ...
    public ExchangeRate findRate(Currency fromCurrency, Currency toCurrency, long date) {
        ...
        Cursor c = db().query(EXCHANGE_RATES_TABLE, ExchangeRateColumns.NORMAL_PROJECTION,
                ExchangeRateColumns.NORMAL_PROJECTION_WHERE,
                new String[]{String.valueOf(fromCurrency.id), String.valueOf(toCurrency.id), String.valueOf(day)},
                null, null, null);
        try {
            if (c.moveToFirst()) {
    ...
    public List<ExchangeRate> findRates(Currency fromCurrency) {
        ...
        Cursor c = db().query(EXCHANGE_RATES_TABLE, ExchangeRateColumns.NORMAL_PROJECTION,
                ExchangeRateColumns.from_currency_id + "=?",
    ...
    public Map<Long, String> getAllAttributesForTransaction(long transactionId) {
        Cursor c = db().query(TRANSACTION_ATTRIBUTE_TABLE, TransactionAttributeColumns.NORMAL_PROJECTION,
                TransactionAttributeColumns.TRANSACTION_ID + "=? AND " + TransactionAttributeColumns.ATTRIBUTE_ID
                        + ">=0",
    ...
    public Cursor getBlotterWithSplits(String where) {
        return db()
                .query(V_BLOTTER_FOR_ACCOUNT_WITH_SPLITS, BlotterColumns.NORMAL_PROJECTION, where, null, null, null,
    ...
    public Cursor getCategoriesWithoutSubtree(long id, boolean includeNoCategory) {
        ...
        long left = 0, right = 0;
        try (Cursor c = db
                .query(CATEGORY_TABLE, new String[]{CategoryColumns.left.name(), CategoryColumns.right.name()},
    ...
    public Category getCategoryWithParent(long id) {
        ...
        try (Cursor c = db.query(V_CATEGORY, CategoryViewColumns.NORMAL_PROJECTION,
            ...
            if (c.moveToNext()) {
                ...
                try (Cursor c2 = db
                        .query(GET_PARENT_SQL, new String[]{CategoryColumns._id.name()}, null, new String[]{s, s},
                                null, null, null, "1")) {
                    if (c2.moveToFirst()) {
    ...
    public ExchangeRateProvider getHistoryRates() {
        ...
        Cursor c = db()
                .query(EXCHANGE_RATES_TABLE, ExchangeRateColumns.NORMAL_PROJECTION, null, null, null, null, null);
    ...
    public List<SmsTemplate> getSmsTemplatesByNumber2(String smsNumber) {
        try (Cursor c = db().rawQuery(
                String.format("select %s from %s where %s=? order by length(%s) desc, %s",
                        DatabaseUtils.generateSelectClause(NORMAL_PROJECTION, null), SMS_TEMPLATES_TABLE, title,
                        template, _id), new String[]{smsNumber})) {
            List<SmsTemplate> res = new ArrayList<>(c.getCount());
            while (c.moveToNext()) {
                SmsTemplate a = SmsTemplate.fromCursor(c);
                res.add(a);
    ...
    public Cursor getSmsTemplatesWithFullInfo() {
        String nativeQuery = String.format(
                ...
                CategoryViewColumns.title, SmsTemplateListColumns.cat_name, CategoryViewColumns.level,
                SmsTemplateListColumns.cat_level,
                SMS_TEMPLATES_TABLE,
                V_CATEGORY,
        ...
        if (nearestTransactionId > 0) {
            ...
            try {
                ...
                db.execSQL(INSERT_RUNNING_BALANCE,
                        new Object[]{account.id, newTransaction.id, newTransaction.dateTime,
                                newTransaction.fromAmount});
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
            }
...
// ===================================================================
    ...
    private long fetchAccountBalance(long accountId) {
        try (Cursor c = db()
                .query(V_BLOTTER_FOR_ACCOUNT_WITH_SPLITS, new String[]{"SUM(" + BlotterColumns.from_amount + ")"},
    ...
    private long getSingleCurrencyId() {
        try (Cursor c = db().rawQuery("select distinct " + AccountColumns.CURRENCY_ID + " from " + ACCOUNT_TABLE +
                        " where " + AccountColumns.IS_INCLUDE_INTO_TOTALS + "=1 and " + AccountColumns.IS_ACTIVE + "=1",
                null)) {
    ...
    private void updateAttribute(Attribute attribute) {
        db().update(ATTRIBUTES_TABLE, attribute.toValues(), AttributeColumns.ID + "=?",
                new String[]{String.valueOf(attribute.id)});
    }
