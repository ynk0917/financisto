...
public class CsvExportTest extends AbstractExportTest<CsvExport, CsvExportOptions> {
    ...
    public void test_should_export_regular_transaction() throws Exception {
        CsvExportOptions options = new CsvExportOptions(createExportCurrency(), ',', false, false, false,
                WhereFilter.empty(), false);
        TransactionBuilder.withDb(db).dateTime(DateTime.date(2011, 8, 3).at(22, 34, 55, 10))
                .account(a1).amount(-123456).category(categoriesMap.get("AA1")).payee("P1").location("Home")
                .project("P1").note("My note").create();
        TransactionBuilder.withDb(db).dateTime(DateTime.date(2011, 8, 4).at(23, 34, 55, 10))
                .account(a1).amount(-789).originalAmount(a2.currency, -888).category(categoriesMap.get("AA1"))
                .payee("P1").location("Home").project("P1").note("My note").create();
        assertEquals(
    ...
    public void test_should_export_split_transaction() throws Exception {
        CsvExportOptions options = new CsvExportOptions(createExportCurrency(), ',', false, true, false,
                WhereFilter.empty(), false);
        TransactionBuilder.withDb(db).dateTime(DateTime.date(2011, 8, 3).at(22, 34, 55, 10))
    ...
    public void test_should_include_header() throws Exception {
        CsvExportOptions options = new CsvExportOptions(Currency.EMPTY, ',', true, false, false, WhereFilter.empty(),
                false);
        assertEquals(
