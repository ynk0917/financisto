...
public class MyPreferences {
    ...
    /**
     * Get the flag that indicates if the category monthly result will consider the result of its sub categories or
     * not.
     *
    ...
    public static int getLockTimeSeconds(Context context) {
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context);
        return isPinLockEnabled(context) ? 60 * Integer
                .parseInt(sharedPreferences.getString("pin_protection_lock_time", "5")) : 0;
    }
        ...
        try {
            ...
            if (currencies != null && currencies.size() > 0) {
                for (Currency currency : currencies) {
                    if (currency.title.equals(refCurrency)) {
                        cur = currency;
                    }
                }
            }
     ...
     *
     * @param context The activity context
     * @return The string representing the currency registered as a reference to display chart reports or null if not
     * configured yet.
     */
    ...
    /**
     * Gets the flag that indicates if the list of filter ids will include No Filter (no category, no project or
    ...
    public static boolean isSetFocusOnAmountField(Context context) {
        return PreferenceManager.getDefaultSharedPreferences(context)
                .getBoolean("ntsl_set_focus_on_amount_field", false);
    }
    ...
    public static boolean isUseFingerprintFallbackToPinEnabled(Context context) {
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context);
        return isPinProtected(context) && sharedPreferences
                .getBoolean("pin_protection_use_fingerprint_fallback_to_pin", true);
    }
    ...
    private static ExchangeRateProviderFactory getExchangeRateProviderFactory(SharedPreferences sharedPreferences) {
        String provider = sharedPreferences
                .getString("exchange_rate_provider", ExchangeRateProviderFactory.freeCurrency.name());
        if ("flowzr".equals(provider)) {
            provider = ExchangeRateProviderFactory.freeCurrency.name();
        }
