...
public class QifExportTest extends AbstractExportTest<QifExport, QifExportOptions> {
    ...
    public void test_should_export_account_with_a_couple_of_transactions() throws Exception {
        ...
        TransactionBuilder.withDb(db).account(a).amount(1000).category(p1).project("PP1").dateTime(date(2011, 2, 8))
                .create();
        TransactionBuilder.withDb(db).account(a).amount(-2056).category(c1).payee("Payee 1").project("PP2")
                .note("Some note here...").dateTime(date(2011, 2, 7)).create();
        assertEquals(
                "!Type:Cat\n" +
                        "NP1\n" +
                        "E\n" +
    ...
    public void test_should_export_transaction_date_according_to_the_config() throws Exception {
        Account a = createFirstAccount();
        TransactionBuilder.withDb(db).account(a).amount(-210056).payee("Payee 1").dateTime(date(2011, 7, 10))
                .create();
        assertEquals(
                "!Account\n" +
                        "NMy Cash Account\n" +
    ...
    public void test_should_export_transfers() throws Exception {
        ...
        TransferBuilder.withDb(db).fromAccount(a2).fromAmount(-2000).toAccount(a1).toAmount(2000)
                .dateTime(date(2011, 2, 8)).create();
        assertEquals(
                "!Account\n" +
                        "NMy Cash Account\n" +
                        "TCash\n" +
    ...
    private String exportAsString() throws Exception {
        QifExportOptions options = new QifExportOptions(Currency.EMPTY, QifExportOptions.DEFAULT_DATE_FORMAT, null,
                WhereFilter.empty(), false);
        return exportAsString(options);
    }

    private String exportAsString(Currency currency) throws Exception {
        QifExportOptions options = new QifExportOptions(currency, QifExportOptions.DEFAULT_DATE_FORMAT, null,
                WhereFilter.empty(), false);
        return exportAsString(options);
    }
    ...
    private String exportAsString(long[] accounts) throws Exception {
        QifExportOptions options = new QifExportOptions(Currency.EMPTY, QifExportOptions.DEFAULT_DATE_FORMAT,
                accounts, WhereFilter.empty(), false);
        return exportAsString(options);
    }
