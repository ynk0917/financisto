...
public class ReportDataByPeriod {
     ...
     */
    public ReportDataByPeriod(Context context, int periodLength, Currency currency, String filterColumn,
            int[] filterId, MyEntityManager em) {
     ...
     * @param dbAdapter    Database adapter to query data
     */
    public ReportDataByPeriod(Context context, int periodLength, Currency currency, String filterColumn, int filterId,
            MyEntityManager em) {
        Calendar startPeriod = Report2DChart.getDefaultStartPeriod(periodLength);
    ...
    public ReportDataByPeriod(Context context, Calendar startDate, int periodLength, Currency currency,
            String filterColumn, int[] filterId, MyEntityManager em) {
    ...
    private void fillEmptyList(Calendar startDate, int periodLength) {
        ...
        for (int index = 0; index < periodLength; index++) {
            month = new GregorianCalendar(startDate.get(Calendar.YEAR), startDate.get(Calendar.MONTH) + index, 1, 0,
                    0, 0);
    ...
    private String getWhereClause(String filterColumn, int[] filterId, int[] accounts) {
        ...
        // period
        accountsWhere
                .append(" and (" + TransactionColumns.datetime + ">=? and " + TransactionColumns.datetime + "<=?)");
    ...
    private String[] getWherePars(Calendar startDate, int periodLength, int[] filterId, int[] accounts) {
        ...
        i++;
        Calendar endDate = new GregorianCalendar(startDate.get(Calendar.YEAR), startDate.get(Calendar.MONTH), 1, 0, 0,
                0);
    ...
    private void init(Context context, Calendar startDate, int periodLength, Currency currency, String filterColumn,
        ...
        try {
            ...
            // query data
            cursor = db.query(TRANSACTION_TABLE, new String[]{filterColumn, TransactionColumns.from_amount.name(),
                            TransactionColumns.datetime.name(), TransactionColumns.datetime.name()},
